    ...
    return (
        <AppLayout title="Legislative Records">
            <PageFrame>
                ...
                <section className="space-y-3" aria-label="Legislative record search and filters">
                    <form onSubmit={search} className="flex gap-2 rounded-xl border border-slate-200 bg-white p-2.5 dark:border-slate-700 dark:bg-[#142236]">
                        <label htmlFor="legislative-search" className="sr-only">Search legislative records</label>
                        <Search className="ml-1 mt-2 text-slate-400" size={17} aria-hidden="true" />
                        <input
                            id="legislative-search"
                            value={q}
                            onChange={(event) => setQ(event.target.value)}
                            className="min-w-0 flex-1 bg-transparent px-1.5 py-2 text-sm text-slate-900 outline-none focus:ring-2 focus:ring-blue-700/20 dark:text-slate-100"
                            placeholder="Search number, title, subject, or keyword"
                        />
                        <button type="submit" className="rounded-lg bg-[#0b2852] px-4 py-2 text-sm font-semibold text-white focus:outline-none focus:ring-2 focus:ring-blue-700/30">Search</button>
                    </form>
