    ...
    return (
        <section className="rounded-xl border border-slate-200 bg-white dark:border-slate-700 dark:bg-[#142236]" aria-label="Session scheduling">
            <div className="flex flex-wrap items-center justify-between gap-2 px-3 py-2.5">
                ...
                <button type="button" aria-expanded={open} aria-controls="legislative-session-form" onClick={() => setOpen((value) => !value)} className="w-full rounded-md bg-[#0b2852] px-3 py-1.5 text-xs sm:w-auto font-semibold text-white focus:outline-none focus:ring-2 focus:ring-blue-700/30">{open ? 'Close' : 'Schedule session'}</button>
            </div>
            {open && (
                <form id="legislative-session-form" onSubmit={submit} className="grid gap-2 border-t border-slate-100 p-3 dark:border-slate-700 sm:grid-cols-2 lg:grid-cols-6">
                    ...
                    <label className="grid gap-1 text-xs font-semibold text-slate-600 dark:text-slate-300 sm:col-span-2"><span>Title</span><input required value={form.data.title} onChange={(e) => form.setData('title', e.target.value)} className="rounded-md border border-slate-300 bg-white px-2.5 py-2 text-sm font-normal text-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-700/25 dark:border-slate-600 dark:bg-slate-950/40 dark:text-slate-100" /></label>
