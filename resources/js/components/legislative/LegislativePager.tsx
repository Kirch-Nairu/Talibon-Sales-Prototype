...
export default function LegislativePager({ page, pageCount, onPageChange, label = 'Results pages' }: Props) {
    ...
    return (
        <nav className="flex flex-wrap items-center justify-between gap-2 border-t border-slate-100 px-4 py-3 dark:border-slate-700" aria-label={label}>
            ...
            <div className="hidden flex-wrap items-center justify-center gap-1 sm:flex">
                {pages.map((item) => (
                    <button
                        ...
                        onClick={() => move(item)}
                        className={`min-w-8 rounded-md px-2 py-1.5 text-xs font-semibold focus:outline-none focus:ring-2 focus:ring-blue-700/30 ${item === page ? 'bg-[#0b2852] text-white' : 'text-slate-600 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800'}`}
                    >
                        {item}
                    </button>
                ))}
