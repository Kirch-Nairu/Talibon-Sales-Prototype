    ...
    return (
        <article className="rounded-xl border border-slate-200 bg-white p-3 dark:border-slate-700 dark:bg-[#142236]">
            <div className="flex flex-col gap-2 @min-[560px]:flex-row @min-[560px]:items-start @min-[560px]:justify-between">
                ...
                <button type="button" aria-expanded={showAgenda} aria-controls={agendaId} onClick={toggleAgenda} className="w-full shrink-0 rounded-md border border-indigo-200 px-3 py-1.5 @min-[560px]:w-auto text-xs font-semibold text-indigo-800 focus:outline-none focus:ring-2 focus:ring-indigo-700/25 dark:border-indigo-900 dark:text-indigo-300">{showAgenda ? 'Hide agenda' : 'View agenda'}</button>
            </div>
            {showAgenda && (
                <div id={agendaId} className="mt-3 border-t border-slate-100 pt-3 dark:border-slate-700">
                    {canManage && (
                        <div className="mb-2.5">
                            <button type="button" aria-expanded={showAgendaForm} aria-controls={agendaFormId} onClick={() => setShowAgendaForm((value) => !value)} className="w-full rounded-md bg-indigo-50 px-3 py-1.5 @min-[560px]:w-auto text-xs font-semibold text-indigo-800 focus:outline-none focus:ring-2 focus:ring-indigo-700/25 dark:bg-indigo-950/30 dark:text-indigo-300">{showAgendaForm ? 'Cancel agenda entry' : 'Add agenda item'}</button>
                            {showAgendaForm && (
                                <form id={agendaFormId} onSubmit={submitAgenda} className="mt-2 grid gap-2 rounded-lg bg-indigo-50/50 p-2.5 dark:bg-indigo-950/20 @min-[620px]:grid-cols-[90px_1fr_auto]">
