...
export default function LegislativeSessionList({ sessions, canManage }: { sessions: LegislativeSession[]; canManage: boolean }) {
    ...
    return (
        <section className="space-y-2" aria-labelledby="legislative-sessions-heading">
            <div className="flex flex-wrap items-end justify-between gap-2"><div><div className="text-xs font-bold uppercase tracking-wide text-indigo-700 dark:text-indigo-300">Session register</div><h2 id="legislative-sessions-heading" className="mt-0.5 text-sm font-bold text-slate-950 dark:text-slate-100">Loaded legislative sessions</h2></div><span className="text-xs text-slate-400" aria-live="polite">Showing {start}–{end} of {sessions.length}</span></div>
            <div className="space-y-2">{visible.map((session) => <LegislativeSessionCard key={session.id} session={session} canManage={canManage} />)}{sessions.length === 0 && <div className="rounded-xl bg-slate-100/70 px-4 py-5 text-center text-sm text-slate-500 dark:bg-slate-900/40 dark:text-slate-400">No legislative sessions are loaded.</div>}</div>
