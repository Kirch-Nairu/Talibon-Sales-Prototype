...
export default function AdministrativeAttention({ workload }: { workload: OfficeWorkload[] }) {
    ...
    return <section className="municipal-panel overflow-hidden" aria-labelledby="dashboard-administrative-attention">
        <header className="flex flex-col gap-2 border-b border-slate-200 px-4 py-3 dark:border-slate-700 sm:flex-row sm:items-start sm:justify-between sm:px-5">
            <div className="min-w-0">
                ...
                <p className="mt-1 text-xs leading-5 text-slate-500 dark:text-slate-400">Only offices with overdue or unassigned work are shown here.</p>
            </div>
        ...
        <div className="divide-y divide-slate-100 dark:divide-slate-700">
            {offices.map((office) => <article key={office.id} className="grid gap-2 px-4 py-3 sm:grid-cols-[minmax(0,1fr)_auto_auto] sm:items-center sm:px-5">
                <div className="min-w-0">
