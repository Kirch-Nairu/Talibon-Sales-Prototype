...
export default function AttentionQueue({
    ...
    return <section className="municipal-panel overflow-hidden" aria-labelledby="dashboard-attention-work">
        ...
        <div className="divide-y divide-slate-100 dark:divide-slate-700">
            {visibleItems.map((item) => <Link key={item.detailUrl} href={item.detailUrl} className="grid min-w-0 gap-2 px-4 py-2.5 transition-colors hover:bg-slate-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-blue-500 dark:hover:bg-slate-800/40 sm:px-5 @min-[760px]:grid-cols-[minmax(0,2fr)_120px_120px_135px] @min-[760px]:items-center">
                <div className="min-w-0">
                    ...
                    <div className="mt-0.5 break-words text-sm font-semibold leading-5 text-slate-950 dark:text-slate-100">{item.title}</div>
                    <div className="mt-0.5 text-xs text-slate-500 dark:text-slate-400">{item.currentOffice?.shortName || item.currentOffice?.name || 'Office not recorded'} · {humanize(item.priority)}</div>
                </div>
        ...
        {items.length > visibleItems.length ? <Link href={href} className="flex items-center justify-between gap-3 border-t border-slate-200 bg-slate-50/60 px-4 py-2 text-xs font-semibold text-blue-700 hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-blue-500 dark:border-slate-700 dark:bg-slate-900/20 dark:text-blue-300 sm:px-5">
            <span>{items.length - visibleItems.length} more attention item{items.length - visibleItems.length === 1 ? '' : 's'}</span>
            <span className="inline-flex items-center gap-1">{linkLabel}<ArrowRight size={13} aria-hidden="true" /></span>
        </Link> : null}
    </section>;
}
