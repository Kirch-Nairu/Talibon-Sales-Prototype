...
export default function OfficeActivityFeed({ activity }: { activity: DashboardOfficeActivity[] }) {
    return <section className="municipal-panel overflow-hidden" aria-labelledby="dashboard-office-activity">
        <DashboardSectionHeader
            ...
            icon={<Activity size={16} className="text-slate-500 dark:text-slate-400" aria-hidden="true" />}
            title="Recent office activity"
            description="Recent cross-office movement retained as operational history."
        />
        <div className="divide-y divide-slate-100 dark:divide-slate-700">
            {activity.slice(0, 4).map((item) => <article key={item.id} className="grid min-w-0 gap-1 px-4 py-2.5 sm:px-5 @min-[620px]:grid-cols-[135px_minmax(0,1fr)_125px] @min-[620px]:items-start @min-[620px]:gap-3">
                <div className="text-xs font-semibold text-slate-600 dark:text-slate-300">{item.office}</div>
                <div className="min-w-0">
                    <div className="text-[11px] font-bold uppercase tracking-wide text-slate-500 dark:text-slate-400">{item.action}</div>
