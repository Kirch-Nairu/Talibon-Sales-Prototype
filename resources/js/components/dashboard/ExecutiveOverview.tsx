    ...
    return (
        <section className="space-y-3.5" aria-labelledby="dashboard-executive-overview">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
                <div>
                    <h3 id="dashboard-executive-overview" className="mt-1 text-base font-bold text-slate-950 dark:text-slate-100">Municipal workload</h3>
                    <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">Current office workload across the municipal organization.</p>
                </div>
                <Link href="/mayor-office" className="inline-flex w-fit items-center gap-1 text-xs font-semibold text-blue-700 hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 dark:text-blue-300">Open executive work <ArrowRight size={13} aria-hidden="true" /></Link>
            </div>
            ...
            <div className="overflow-hidden rounded-xl border border-slate-200 bg-white dark:border-slate-700 dark:bg-[#142236]">
                <div className="flex flex-col gap-1 border-b border-slate-100 px-4 py-3 dark:border-slate-700 sm:flex-row sm:items-center sm:justify-between sm:px-5">
                    <div className="flex items-center gap-2"><Building2 size={16} className="text-blue-700 dark:text-blue-300" aria-hidden="true" /><h4 className="text-sm font-bold text-slate-950 dark:text-slate-100">Office workload</h4></div>
                    <span className="text-xs text-slate-500 dark:text-slate-400">Overdue and unassigned work first</span>
                </div>
                ...
                <div className="divide-y divide-slate-100 dark:divide-slate-700">
                    {overview.departmentWorkload.map((office) => (
                        <article key={office.id} className="grid gap-2 px-4 py-3 sm:px-5 @min-[700px]:grid-cols-[minmax(0,1.5fr)_repeat(4,82px)] md:items-center">
                            ...
                            <div className="text-xs text-slate-600 dark:text-slate-300"><span className="font-bold text-slate-950 dark:text-slate-100">{office.active}</span> <span className="@min-[700px]:hidden">active</span></div>
                            ...
                            <div className="text-xs text-amber-700 dark:text-amber-300">{office.dueSoon} <span className="@min-[700px]:hidden">due soon</span></div>
                            <div className={`inline-flex items-center gap-1 text-xs ${office.overdue > 0 ? 'font-semibold text-rose-700 dark:text-rose-300' : 'text-slate-500 dark:text-slate-400'}`}><AlertTriangle size={11} aria-hidden="true" /> {office.overdue} overdue</div>
                        </article>
                    ))}
